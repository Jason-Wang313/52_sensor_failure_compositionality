...
Submission-hardening version: v2. Decision: workshop-only. The original pairwise diagnostic is useful, but it is not a universal composition law. The v2 stress adds a hidden high-order common-cause interaction and shows that a pairwise model can extrapolate badly to held-out triple-failure cells.

\section{Synthetic Evidence}

We ran a controlled synthetic experiment with 800 samples per regime, for 2,400 total trials. The independent regime sets all interaction penalties to zero. The mild and strong regimes add increasingly large pairwise penalties. We compare an additive predictor to an interaction-aware predictor that is given the correct composed form.
...
\caption{V2 higher-order stress on the held-out triple-failure cell. A pairwise law fails when the true degradation contains an unmodeled high-order common-cause interaction.}
\label{tab:v2-higher-order}
\end{table}
...
The pattern is deliberately simple. When failures are independent, the additive view is exact. Once interactions appear, additive prediction error increases from 0.0927 to 0.1696. The compositional predictor has zero error in this controlled diagnostic because its hypothesis class matches the data-generating law. Table~\ref{tab:v2-higher-order} is the hostile follow-up: when a held-out triple-failure cell has an unmodeled common-cause term, the pairwise law's MAE rises to 0.284 under a mild hidden triple and 0.758 under a strong hidden triple. The point is not that real robots will have zero-error pairwise laws; the point is that the additive assumption is empirically testable, but the tested composition order must be stated and stressed.
...
The evidence here is synthetic and therefore cannot establish transfer to all robot perception systems. The paper is strongest as a mechanism note and weakest as a deployment claim. The v2 stress also shows that a low-order composition law can be overconfident if higher-order common-cause failures are omitted. A real-robot follow-up should evaluate camera-depth-LiDAR-tactile combinations under paired and higher-order corruptions, calibration drift, and asynchronous dropout, then fit the interaction terms actually observed.
